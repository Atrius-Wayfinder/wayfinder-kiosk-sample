...
export const VirtualKeyboard = memo(function VirtualKeyboard() {
  ...
  const { isVisible, inputValue, keyboardPosition, updateValue, hideKeyboard } = useKeyboard();
  ...
  // Build inline styles for dynamic positioning
  const keyboardStyle: React.CSSProperties = keyboardPosition.top !== undefined
    ? { top: `${keyboardPosition.top}px` }
    : { bottom: `${keyboardPosition.bottom ?? 24}px` };
  ...
  return (
    <>
      ...
      {/* Keyboard Modal */}
      <div
        className="fixed left-1/2 -translate-x-1/2 z-50 bg-gradient-to-b from-gray-50 to-gray-100 border-2 border-gray-300 rounded-2xl shadow-2xl max-w-2xl w-auto"
        style={keyboardStyle}
        role="application"
