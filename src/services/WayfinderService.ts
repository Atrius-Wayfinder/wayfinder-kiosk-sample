...
class WayfinderService {
  ...
  // Cache for floor ID to name mapping (populated from SDK getStructures())
  private floorNameCache: Map<string, string> | null = null;
  private floorNameCachePromise: Promise<Map<string, string>> | null = null;
  ...
  /**
   * Build floor name cache from SDK getStructures() response
   * Creates a Map of floorId -> floorName for quick lookups
   */
  private async buildFloorNameCache(): Promise<Map<string, string>> {
    // Return cached result if available
    if (this.floorNameCache) {
      return this.floorNameCache;
    }

    // Return in-progress promise if already building
    if (this.floorNameCachePromise) {
      return this.floorNameCachePromise;
    }

    // Start building the cache
    this.floorNameCachePromise = (async () => {
      const cache = new Map<string, string>();

      try {
        const map = await this.waitForInstance();
        const structures = await map.getStructures();

        // Build lookup from all structures and their levels
        for (const structure of structures) {
          for (const level of structure.levels) {
            // Map levelId to levelName (e.g., "llia-terminal1-departures" -> "Terminal 1 Departures")
            cache.set(level.id, level.name);
          }
        }

        console.log(`Floor name cache built with ${cache.size} entries`);
        this.floorNameCache = cache;
        return cache;
      } catch (error) {
        console.error('Failed to build floor name cache:', error);
        // Return empty cache on failure - callers will use fallback
        return cache;
      }
    })();
  ...
  /**
   * Get human-readable floor name from floor ID
   * Uses SDK getStructures() data with fallback to regex parsing
   * @param floorId - The floor ID (e.g., "llia-terminal1-departures")
   * @returns Human-readable floor name (e.g., "Terminal 1 Departures")
   */
  async getFloorName(floorId: string | undefined): Promise<string> {
    if (!floorId) {
      return 'Unknown Location';
    }

    try {
      const cache = await this.buildFloorNameCache();
      const name = cache.get(floorId);
    ...
    // Fallback: use regex parsing if SDK lookup fails
    // Import dynamically to avoid circular dependency
    const { parseFloorId } = await import('@/utils/floorParser');
    return parseFloorId(floorId);
  }

  /**
   * Synchronous floor name lookup - uses cached data only
   * Returns null if cache not yet populated (caller should use fallback)
   * @param floorId - The floor ID
   * @returns Floor name if cached, null otherwise
   */
  getFloorNameSync(floorId: string | undefined): string | null {
    if (!floorId || !this.floorNameCache) {
      return null;
    }
    return this.floorNameCache.get(floorId) ?? null;
  }
}
