...
class DirectoryService {
  ...
  private async fetchAllPOIs(): Promise<SDKPOI[]> {
    ...
    if (Array.isArray(results)) {
      ...
      // Map to array, only including POIs with stable IDs
      // Never use index-based or synthetic IDs - that breaks routing
      return poiResults
        .filter((poi) => {
          const hasStableId = !!(poi.id || poi.poiId);
          if (!hasStableId) {
            console.warn('DirectoryService: Excluding POI without stable ID:', poi.name);
          }
          return hasStableId;
        })
        .map((poi) => ({
  ...
  private extractSecurityWaitTimes(pois: (SDKPOI | DirectoryPOI)[]): SecurityWaitTime[] {
    ...
    // Map to simplified wait time objects, excluding any without stable IDs
    return securityPOIs
      .filter((poi) => {
        const hasStableId = !!(poi.id || poi.poiId);
        if (!hasStableId) {
          console.warn('DirectoryService: Excluding security POI without stable ID:', poi.name);
        }
        return hasStableId;
      })
      .map((poi) => ({
